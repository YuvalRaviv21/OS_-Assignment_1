...
void exit(int status, char *exit_msg) // TODO 3.1 Exit message
{
  struct proc *p = myproc();
  ...
  p->xstate = status;
  // Copy exit message into p->exit_msg
  if (exit_msg == 0)
  {
    strncpy(p->exit_msg, "No exit message", sizeof(p->exit_msg) - 1);
  }
  else
  {
    strncpy(p->exit_msg, exit_msg, sizeof(p->exit_msg) - 1);
  }
...
int wait(uint64 addr, uint64 addr2) // TODO 3.3
{
  struct proc *pp;
  int havekids, pid;
  struct proc *p = myproc();
    ...
    for (pp = proc; pp < &proc[NPROC]; pp++)
    {
      if (pp->parent == p)
      {
        ...
        if (pp->state == ZOMBIE)
        {
          ...
          if (addr2 != 0 && copyout(p->pagetable, addr2, pp->exit_msg, sizeof(pp->exit_msg)) < 0)
          {
            release(&pp->lock);
            release(&wait_lock);
            return -1;
